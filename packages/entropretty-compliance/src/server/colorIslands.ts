    ...
    try {
      ...
      const floodFill = (start: Point, baseColor: number[]): ColorIsland => {
        ...
        const isColorMatch = (x: number, y: number): boolean => {
          ...
          const pixelColor = Array.from(rawData.slice(idx, idx + channels!));
      ...
      for (let y = 0; y < height!; y++) {
        for (let x = 0; x < width!; x++) {
          ...
          const idx = (y * width! + x) * channels!;
          const color = Array.from(rawData.slice(idx, idx + channels!));
      ...
      const metadata: CheckMetadata[] = failingIslands.map((island) => {
        ...
        const heightPercent = ((boundingRect.height / height!) * 100).toFixed(
          1
        );
